...
fn check_string_number(line: &str, number: &mut i32, index: usize) {
    if line.chars().nth(index).unwrap() == 'o' {
        if get_substring(line, index, "one".len()).map(|s| s.to_string()).unwrap_or_default() == "one" {
            *number = 1;
        }
    }
    ...
    if line.chars().nth(index).unwrap() == 't' {
        ...
        if get_substring(line, index, "two".len()).map(|s| s.to_string()).unwrap_or_default() == "two" {
            *number = 2;
        }

        if get_substring(line, index, "three".len()).map(|s| s.to_string()).unwrap_or_default() == "three" {
            *number = 3;
        }
    }
    ...
    if line.chars().nth(index).unwrap() == 'f' {
        ...
        if get_substring(line, index, "four".len()).map(|s| s.to_string()).unwrap_or_default() == "four" {
            *number = 4;
        }
        
        if get_substring(line, index, "five".len()).map(|s| s.to_string()).unwrap_or_default() == "five" {
            *number = 5;
        }
    }
    ...
    if line.chars().nth(index).unwrap() == 's' {
        ...
        if get_substring(line, index, "six".len()).map(|s| s.to_string()).unwrap_or_default() == "six" {
            *number = 6;
        }

        if get_substring(line, index, "seven".len()).map(|s| s.to_string()).unwrap_or_default() == "seven" {
            *number = 7;
        }
    }
    ...
    if line.chars().nth(index).unwrap() == 'e' {
        ...
        if get_substring(line, index, "eight".len()).map(|s| s.to_string()).unwrap_or_default() == "eight" {
            *number = 8;
        } 
    }
    ...
    if line.chars().nth(index).unwrap() == 'n' {
        ...
        if get_substring(line, index, "nine".len()).map(|s| s.to_string()).unwrap_or_default() == "nine" {
            *number = 9;
        }    
    }
}
